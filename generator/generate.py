...
def _import(name):
    mname = name.lower()
    cname = name.title()
    mod = __import__('deepo.generator.modules.%s' % mname, fromlist=[cname])
...
def deepo_generator(args):
    ...
    for module in args['modules']:
        ...
        if len(terms) > 1:
            versions[m] = terms[1]
    composer = Composer(in_modules, args['cuda_ver'], args['cudnn_ver'], args['ubuntu_ver'], args['workspace'], versions)
